...
import sys
from world import obstacles#import obstacles module to use functions and clear list 


if len(sys.argv) > 1:#imports module depending on users input via the command line
...
def do_replay(robot_name, arguments):
    ...
    for (command_name, command_arg) in commands_to_replay:
        ...
        if not silent:
            ...
            print(world.show_position(robot_name))#printed show position too fix replay related failures
...
def robot_start():
    ...
    #very important for unittesting to clear list of obsticales
